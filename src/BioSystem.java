...
//        }
    ...
    static void replicateFigure4Solo(String fileID, int nCores, int nBlocks, double[] rate_ratios){
        ...
        // TODO - INCREASED duration TO 10,000 FOR THE veryLong SIMULATIONS
        double duration = 10000.; //100 hour duration
        ...
        String results_directory = "/Disk/ds-sopa-personal/s1212500/multispecies-sims/biofilm_threshold_theory_v2/solo_results_bigK_veryLong_v2";
        String pop_filename = fileID+"-stochastic_pop_over_time_veryLong_v2"; //file to save all the populations over time
        String microhab_filename = fileID+"-stochastic_microhabs_over_time_veryLong_v2"; //file to save the times at which new microhabs are created
    ...
    private static DataBox replicateFigure4_subroutine(double duration, int nSamples, int runID, int K, double[] rate_ratios){
        ...
        while(bs.time_elapsed < duration+0.2*interval){
            ...
            if((bs.getTimeElapsed()%interval <= 0.1*interval) && !alreadyRecorded){
                ...
                System.out.println("runID: "+runID+"\t"+String.format("t: %.3f \tN: %d", bs.time_elapsed, bs.getTotalN()));//t: "+bs.time_elapsed);
                times.add(bs.time_elapsed);
                total_pop_over_time.add((double)bs.getTotalN());
    ...
    public static void stochasticWaitingTime(String fileID, int nCores, int nBlocks, double[] rate_ratios){
        ...
        int thickness_limit = 3;
        int K = 10000;
        double duration = 5e3; //very long duration, this is only to make sure that we don't miss any datapoints
        int nSamples = 1024; //this is just used to print output now
        ...
        String pop_filename = fileID+"-stochastic_pop_over_time_run2"; //file to save all the populations over time
        String microhab_filename = fileID+"-stochastic_microhabs_over_time_run2"; //file to save the times at which new microhabs are created
